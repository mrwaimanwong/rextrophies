...
class VPC_Updating_Manager {
				...
				'</div><div>'
                        ...
                        if($information){
                            ...
                            $formatted_info->sections=array(
                                "description"=>"A simple lettering & signs design plugin for WooCommerce.",
                            ...
                            $formatted_info->sections=array(
                                "description"=>"A simple lettering & signs design plugin for WooCommerce.",
                                "changelog"=>'<div>' . __( "Unable to retrieve update details.", "vpc" ) . '</div>');
                        }
	...
	public function addUpgradeMessageLink() {
            ...
            if (isset($options['license-key']) && $options['license-key'] != "") {
                    echo '<a href="' . wp_nonce_url( admin_url( 'update.php?action=upgrade-plugin&plugin='.VPC_MAIN_FILE ), 'upgrade-plugin_'.VPC_MAIN_FILE ) . '">' . __( 'Update Lettering & Signs Designer now.', 'vpc' ) . '</a>';
            }else{
                $url = admin_url('admin.php?page=vpc-manage-settings');
                echo ' <a href="' . $url . '">' . __( 'A license key is needed to update this plugin.', 'vpc' ) . '</a>';
            }
	}
}
